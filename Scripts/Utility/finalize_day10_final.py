...
def finalize_leaderboard():
    ...
    "days": {
      ...
      "D10": { "pts": 270, "submitted": true, "streakDays": 4, "workDone": true, "workStreakDays": 9 }
    },
    "breakdown": [
      { "section": "PRE-PROGRAMME", "items": [
          { "label": "Pledge", "pts": 5, "earned": true, "dayHits": null, "desc": "You committed to the programme. +5 for your pledge." },
      ...
      { "section": "WORK POST", "items": [
          { "label": "Work post (image uploaded)", "pts": 5, "earned": true, "dayHits": ["D1", "D2", "D3", "D4", "D5", "D6", "D7", "D8", "D10"], "desc": "D1:5 · D2:5 · D3:5 · D4:5 · D5:5 · D6:5 · D7:5 · D8:5 · D10:10. Total: +50." }
      ]},
      { "section": "WHATSAPP ENGAGEMENT", "items": [
    ...
    content = re.sub(r'\{[^{]*?"name": "Christine Choundong".*?\}\s*\],', christine_new, content, flags=re.DOTALL)
    content = re.sub(r'\{[^{]*?"name": "Abongnwi Chrioni-Opal Forba'".*?\}\s*\],', abongnwi_new, content, flags=re.DOTALL)
    content = re.sub(r'\{[^{]*?"name": "Amaazee Ivanna Therese Fundoh".*?\}\s*\],', amaazee_new, content, flags=re.DOTALL)

    # 4. Explicit Work Post Descriptions for everyone else
    # Find all "Work post (image uploaded)" blocks
